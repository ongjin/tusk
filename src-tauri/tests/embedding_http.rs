...
use tusk_lib::db::embedding_http::{embed_gemini_at, embed_ollama_at, embed_openai_at};
...
async fn ollama_roundtrip() {
    ...
    let r = embed_ollama_at(&Client::new(), &url, "nomic-embed-text", "hello")
        .await
        .unwrap();
